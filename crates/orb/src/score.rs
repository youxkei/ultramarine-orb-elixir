//! Keeping a chapter-mode run's scores out of the game's own file.
//!
//! A run that has been rewound is not a run anyone played, so its score does not belong in
//! the game's ranking — the same reason replay files are not written. Refusing the write
//! would lose the record altogether, so the file is forked instead: every open of
//! `score.dat` becomes an open of `orb_score.dat`. Chapter-mode runs are then ranked against
//! each other, in the game's own format and on its own screen, and the game's file comes out
//! of a session unchanged because it is never opened at all.
//!
...
/// orb's, beside it, so that a directory listing says which is which.
const OURS: &[u8] = b"orb_score.dat";
...
///
/// The whole file name is compared rather than its end, which is what keeps `orb_score.dat`
/// from being taken for the game's file and forked again. The directory the game named is
...
mod tests {
    ...
    fn nothing_else_is_forked() {
        for path in [
            ...
            assert!(forked(path).is_none(), "{}", text(path));
        }
    }

    /// orb's own file ends in the game's name, and is not itself forked: what is compared is
    /// the whole file name.
    #[test]
    fn orbs_own_file_is_left_alone() {
        assert!(forked(b"orb_score.dat").is_none());
        assert!(forked(br"C:\game\orb_score.dat").is_none());
    }
