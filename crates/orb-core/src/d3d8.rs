...
//!
//! Nothing here is a call, only the shape of one: the layouts, their slot asserts, and the
//! constants. So it needs no seam and names no Windows — a `Device` is a pointer to a vtable, and a
//! vtable of Rust functions *is* a device. The recording device the tests draw into is exactly that,
//! and it lives in `orb/recording.rs` rather than here, beside the drawing it stands in for: its
//! `Screen` fixture builds a real `Overlay`, which reaches `text.rs` and the GDI.
//!
//! The padding fields are `pub` for its sake — it declares whole vtables, and a field it cannot
//! name is a slot it cannot leave empty.
