...
//! `orb::stage_begun` before it writes the supervisor's copy, so at the moment a stage's numbers go in the
...
/// [`INVULNERABLE_AFTER_SPAWNING`] updates — see `scenario_the_player_a_stage_starts.rs` — so a hit inside
...
fn a_stage_transition_makes_no_read_of_the_score_file() {
    in_its_own_process(|| {
        ...
        // about — see `scenario_the_score_file.rs`, which is where which file each read lands in is.
        game.at_the_title_menu();
