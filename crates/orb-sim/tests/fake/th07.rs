...
impl Fake {
    ...
    pub fn attach(display: Display, name: &str, work: Work) -> Box<Self> {
        ...
        // The font goes, now that the harness's own device has baked its own from it and before orb
        // reaches for one: **a 妖々夢 install has no `font.ttf` beside its exe.** 紅魔郷 ships one and
        // 妖々夢 keeps its fonts inside `th07.dat`, so orb's overlay cannot be built there — measured on
        // the machine as `overlay: cannot load …\font.ttf` eight times and then `overlay: unavailable`.
        // A scenario with a font in its directory would have orb drawing things no launch here draws.
        std::fs::remove_file(launch.dir().join("font.ttf")).expect("the font `scratch` put there");
...
extern "fastcall" fn own_render(_window: *mut c_void) -> i32 {
    ...
    unsafe { orb::run_draw_chain(chain) };
    let walked = unsafe { orb::run_calc_chain(chain) };
