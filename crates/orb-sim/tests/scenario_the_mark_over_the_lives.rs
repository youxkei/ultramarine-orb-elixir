...
use fake::th06::{Fake, PANEL_FRAMES, lives_row, the_run};
...
fn the_mark_survives_a_stage_transition_and_the_frame_a_chapter_is_put_back_on() {
    in_its_own_process(|| {
        ...
        // the same two things `scenario_pointdevice_run.rs` reads of the mark, on this frame.
        assert!(
            game.drawn()
                .quads
                .iter()
...
fn a_run_given_up_at_orbs_own_menu_needs_no_frame_after_it() {
    in_its_own_process(|| {
        ...
        // the first: the same walk `scenario_pointdevice_run.rs` makes.
        game.hit();
        game.frame();
        game.frames(READS_KEYS_AFTER);
        game.press(keys::UP);
