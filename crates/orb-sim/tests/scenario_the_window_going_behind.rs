...
//! frame is paced and the game asked for nothing at all — is `scenario_pacing.rs`'s
//! `a_frame_with_the_window_behind_takes_its_turn_on_the_blanks`.

mod fake;

use fake::th06::{Fake, the_run};
use fake::{Launched, WINDOW, in_its_own_process};
use orb_api::Hwnd;
...
/// `scenario_keys_from_another_program.rs` — and what is left is `GetKeyboardState`, which the system
