# 5. Every e2e test lives in `orb-sim`'s `tests/`, and its file name says it is one
...
were twenty-three binaries. Both are gone: the e2e tests are `#[cfg(test)]` modules of
...
**And the word is `e2e test` now**, the crate's name being what says the level. This document is written
in it throughout — the file names below are the ones that were there, `scenario_*.rs`, and they are left
as they were.
...
[0003](0003-the-frame-loops-e2e-tests-are-one-file.md) and
...
`game::th07` and is a whole launch. `orb/tests/pacing.rs` — 46 e2e tests over orb's own frame loop — and
...
> **A unit test of the log, and not an e2e test**, which is why it is still here and not among the runs in
> `orb/tests`.
...
- One binary per file, as before. `in_its_own_process` spawns the binary it is in, so an e2e test owns a
  process wherever the file sits and nothing about the process-per-e2e test changes.
...
- **A subdirectory of `tests/` holding them.** Cargo auto-discovers `tests/*.rs` and `tests/*/main.rs`, so
  seven files under one would each need a `[[test]]` entry carrying its path — a manifest to keep in step
  with the directory, for the same thing a prefix says with nothing to keep in step.
- **Two letters rather than a name.** The arrangement is borrowed from another project of this author's,
  where the full-stack suite lives in the simulator crate and the layer is two letters in the file name
  (`lb.rs` for the backend alone, `lf.rs` for the whole runtime). The prefix here says the level in the
  name this repository uses for it — `SPEC.md`, every decision here and `fake/mod.rs` all use that name,
  and `log_writes.rs` draws the contrast in it — which is what `CLAUDE.md` asks for.
...
e2e tests, four tests over the simulated Windows with no game in them, and 192 in `#[cfg(test)]` modules.
